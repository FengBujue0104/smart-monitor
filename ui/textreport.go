...
func buildTextReport(disks []smart.Disk, vs []health.Violation) string {
	...
	for _, d := range disks {
		b.WriteString(fmt.Sprintf("[磁盘 %d] %s  (%s)  容量:%.1f GB  S/N:%s  FW:%s\n",
			d.Index, d.Model, d.Kind, d.SizeGB, d.Serial, d.Firmware))
		if len(d.Attrs) == 0 {
		...
		for _, a := range d.Attrs {
			...
			b.WriteString(fmt.Sprintf("  %s 0x%02X %-30s  raw=%s  val=%s worst=%s thresh=%s\n",
				mark, a.ID, a.Name, attrRawStr(a), attrCurrentStr(a), attrWorstStr(a), threshStr(a.Thresh)))
		}
		b.WriteString("\n")
	}
